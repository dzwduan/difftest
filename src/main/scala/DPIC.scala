...
class DPIC[T <: DifftestBundle](gen: T, config: GatewayConfig) extends ExtModule
  ...
  val modPorts: Seq[Seq[(String, Data)]] = {
    ...
    if (config.isBatch) {
      common += Seq(("batch_idx", batch_idx.get))
    }
  ...
  val dpicFuncProto: String =
    s"""
       ...
       |)""".stripMargin
  val dpicFunc: String = {
    val dut_pos = if (config.isBatch) "batch_idx" else "0"
...
private class DummyDPICWrapper[T <: DifftestBundle](gen: T, config: GatewayConfig) extends Module {
  ...
  if (config.isBatch) dpic.batch_idx.get := batch_idx.get
  dpic.io := unpack
}
...
object DPIC {
  ...
  def apply[T <: DifftestBundle](gen: T, config: GatewayConfig, port: GatewayBundle): UInt = {
    ...
    if (config.isBatch) module.batch_idx.get := port.batch_idx.get
  ...
  def collect(config: GatewayConfig): Seq[String] = {
    ...
    interfaceCpp +=
      s"""
         ...
         |private:
         |  DiffTestState buffer[${config.dutBufSize}];
         |  int read_ptr = 0;
         |public:
         |  DPICBuffer() {
         ...
         |    read_ptr = (read_ptr + 1) % ${config.dutBufSize};
         |    return ret;
         |  }
         |};
         |""".stripMargin
    ...
    interfaceCpp += ""
    interfaceCpp += "DiffStateBuffer** diffstate_buffer = nullptr;"
